...
- **A 401 means signed out; anything else does not — and since item 161, only if the answer could be about our own token.** The access token lives 15 minutes, so a 401 is the expected steady state, not a failure — `authRefreshInterceptor` refreshes once and replays when the gateway or `professionalservice` answered, or when the stored token is already stale. A 401 from a sibling service is that product's decision about that one call and buys no rotation. A network drop, timeout or 5xx is propagated untouched, so losing signal mid-refresh surfaces as a failed request rather than a sign-out.
...
| `src/app/shared/completion-meter.component.ts`                                                                                   | `app/account/profile/completion-meter.component.ts` | `639321a`     | 2026-08-22 | **Behavioural, not styling.** Ionic/Tailwind markup rather than the desktop card, and the eight requirement labels moved into `me.completion.requirements.*`. What must not drift is the rule it embodies: it renders the server's figure and **derives nothing** — the same number gates the transition to ACTIVE, so a locally computed one can read 100% while the service still refuses                                                                                                                                                                                                                                                                                                   |
...
| `src/app/core/interceptor/auth-refresh.interceptor.ts` — the `services/<name>` reading and `SESSION_AUTHORITATIVE_SERVICES` only | `app/core/interceptor/auth-expired.interceptor.ts`  | `087e16b`     | 2026-09-18 | **A rule, not a file.** The two interceptors do different things with the answer — `web/` decides whether to sign out, this one whether to refresh — so neither body is copyable. What must not drift is `SESSION_AUTHORITATIVE_SERVICES`: the day a second service starts validating this gateway's token, both lists move or the two clients disagree about the same 401. Two deliberate divergences, argued at the point of use: the regex is anchored after this app's absolute prefix rather than on any `/services/` segment, and a foreign 401 **is** refreshed through when the store already knows the access token is stale — 15 minutes is not 24 hours. Backlog items 156 and 161 |

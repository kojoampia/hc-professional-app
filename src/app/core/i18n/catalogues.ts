/**
 * The four translation catalogues, compiled into the bundle rather than fetched.
 *
 * `web/` loads its `i18n/*.json` over HTTP through `@ngx-translate/http-loader`. That is wrong here
 * for two reasons. The app is offline-first — the Today tab is expected to render from the encrypted
 * cache with no network at all — and a fetched catalogue means the first paint after a cold start on
 * a plane is either untranslated or blocked on a request that will never complete. And it would add
 * a dependency for what is a few kilobytes of static text; `mobile-app-plan.md` already names
 * dependency count as this app's most fragile property.
 *
 * The cost is that a language change ships in a release rather than a deploy, which for four
 * languages that change rarely is the better trade.
 *
 * **Every catalogue must have identical keys.** ngx-translate renders a missing key as the key
 * itself — `me.title` in the middle of a screen — rather than failing, so drift is invisible until
 * someone switches language. `catalogues.spec.ts` compares the key sets and is the gate.
 */

export const EN = {
  common: {
    save: 'Save',
    cancel: 'Cancel',
    retry: 'Retry',
    loading: 'Loading…',
    offline: 'You are offline',
    ageNever: 'never',
    ageJustNow: 'just now',
    ageMinutes: '{{count}} min ago',
    ageHours: '{{count}} h ago',
    ageDays: '{{count}} d ago',
    pendingUnsent: 'Not sent yet',
    pendingConflict: 'Changed elsewhere',
    pendingRejected: 'Refused',
    pendingExpired: 'Gave up',
  },
  /**
   * The sign-in screen. Its strings live here rather than beside the page because four of them are
   * raised from TypeScript — an expired session, a changed biometric enrolment, a rejected
   * credential, an unreachable gateway — and a message that is only translated when it happens to
   * be in a template is worse than one that is never translated at all.
   */
  auth: {
    forgotPassword: 'Forgotten your password?',
    resetTitle: 'Reset your password',
    resetIntro: 'Enter your email address and we will send you a reset key.',
    email: 'Email address',
    resetSend: 'Send the key',
    resetFinishTitle: 'Already have a key?',
    resetFinishIntro: 'The email opens in your browser, so copy the key from it and paste it here.',
    resetKey: 'Reset key',
    newPassword: 'New password',
    resetFinish: 'Set the new password',
    resetClose: 'Back to sign in',
    resetNeedsEmail: 'Enter your email address.',
    resetRequested: 'If that address is registered, a reset key is on its way.',
    resetNeedsKey: 'Paste the key from the email.',
    passwordLength: 'A password must be between 4 and 100 characters.',
    resetDone: 'Your password has been changed. You can sign in with it now.',
    resetBadKey: 'That key was not recognised. It may have expired — ask for a new one.',
    subtitle: 'Professional',
    username: 'Username',
    password: 'Password',
    signIn: 'Sign in',
    noScreenLock: 'This device has no screen lock, so Abofonsa BridgeCare will ask for your password each time you open it.',
    sessionExpired: 'Your session ended. Please sign in again.',
    biometryChanged: "This device's biometric settings changed, so you need to sign in again.",
    badCredentials: 'That username and password did not match.',
    unreachable: 'Could not reach Abofonsa BridgeCare. Check your connection and try again.',
  },
  /** The cold-start splash: the first thing anyone sees, and until now the last thing translated. */
  boot: {
    restoring: 'Restoring your session',
    offline: 'Could not reach Abofonsa BridgeCare. Your session is still valid.',
    tryAgain: 'Try again',
    signInWithPassword: 'Sign in with password',
    unlockPrompt: 'Unlock to continue',
    unlock: 'Unlock',
    unlockReason: 'Unlock Abofonsa BridgeCare Professional',
    unlockTitle: 'Unlock Abofonsa Pro',
    usePassword: 'Use password',
  },
  today: {
    title: 'Today',
    savedData: 'Showing saved data',
    savedDataOffline: 'Offline — showing saved data',
    updated: 'updated',
    finishApplication: 'Finish your application',
    notActive: 'Your application is not active yet, so there is nothing rostered.',
    continueOnWeb: 'Continue at professional.abofonsa.com to complete it.',
    needsAttention: 'Needs attention',
    unreadMessages: 'Unread messages',
    licence: 'Licence',
    expired: 'Expired',
    expires: 'Expires',
    lapsed: 'lapsed',
    nextSevenDays: 'Next 7 days',
    rosterFailed: 'Could not load your roster.',
    nothingRostered: 'Nothing rostered in the next 7 days.',
    noShiftAssigned: 'No shift assigned',
    onDuty: 'On duty',
    nextShift: 'Next shift',
    nothingToday: 'Nothing today',
    flexibleToday: 'Flexible hours today',
    nothingTodayNamed: 'Nothing today, {{name}}',
    until: 'Until {{time}}',
    flexibleOn: 'Flexible on {{date}}',
    dateAtTime: '{{date}} at {{time}}',
  },
  messages: {
    compose: 'New message',
    toPeople: 'To people',
    toRole: 'To a role',
    selected: 'Selected',
    searchRecipients: 'Search by name',
    searchHint: 'Type at least two letters to search.',
    subject: 'Subject',
    body: 'Message',
    composeSend: 'Send',
    composeQueued: 'Saved on this phone. It will be sent when you have signal.',
    composeNeedsBody: 'Write a message before sending.',
    composeNeedsRecipient: 'Choose at least one recipient, or a role.',
    composeRoleEmpty: 'Nobody currently holds that role, so this message would reach no one.',
    composeFailed: 'The message could not be queued.',
    broadcastConfirm: 'Send to everyone with this role?',
    broadcastCount: 'This goes to {{count}} people holding {{role}}.',
    title: 'Messages',
    savedData: 'Showing saved messages',
    savedDataOffline: 'Offline — showing saved messages',
    noSubject: 'No subject',
    loadFailed: 'Could not load messages.',
    empty: 'No messages.',
    close: 'Close',
    threadEmpty: 'No messages in this thread yet.',
    sendFailed: 'Could not send. Check your connection and try again.',
    conversation: 'Conversation',
    send: 'Send',
    /**
     * The byline under one's own message — first person in all four, not second.
     *
     * English chat apps usually say "You", but translating that literally gives `Tú`/`Vous`/`Sie`,
     * which reads as addressing the other person rather than naming yourself, and drags in a
     * formality choice this app has no reason to make. `Me`/`Yo`/`Moi`/`Ich` is what the label
     * means. Owner's decision, 2026-08-14 — do not "correct" it back to second person.
     */
    me: 'Me',
    replyPlaceholder: 'Write a reply',
  },
  documents: {
    title: 'Documents',
    savedData: 'Showing saved documents',
    savedDataOffline: 'Offline — showing saved documents',
    yours: 'Your documents',
    loadFailed: 'Could not load your documents.',
    empty: 'No documents yet.',
    add: 'Add a document',
    type: 'Type',
    choose: 'Choose',
    label: 'Label',
    labelPlaceholder: 'What is this document?',
    expiryDate: 'Expiry date',
    takePhoto: 'Take a photo',
    choosePdfOrImage: 'Choose a PDF or image',
    privacyNote: 'Photos are converted to JPEG and location data is removed before upload.',
    preparing: 'Preparing the image…',
    uploading: 'Uploading…',
    uploaded: 'Uploaded',
    pendingReview: 'It will show as pending until an administrator verifies it.',
    done: 'Done',
    labelRequired: 'Give this document a label.',
    expiryRequired: 'A licence needs its expiry date.',
    uploadPhotoTooLarge: 'That photo is too large even after compression. Try again with less of the background in frame.',
    uploadPhotoUnreadable: 'Could not read that image. Try taking the photo again, or choose a PDF.',
    uploadTypeRejected: 'Only PDF, PNG and JPEG documents are accepted.',
    uploadPdfTooLarge: 'That PDF is larger than 5 MB. Try a smaller scan.',
    uploadImageTooLarge: 'That image is too large even after compression.',
    uploadImageUnreadable: 'Could not read that image.',
    uploadFailed: 'Upload failed. Check your connection and try again.',
    addOrRenew: 'Add or renew a document',
    expires: 'Expires',
    /** Backlog item 45. Mirrors web's healthConnect.onboarding.superseded, same word. */
    superseded: 'Replaced',
    /**
     * The status badge was `verificationStatus.toLowerCase()` — raw English on every locale, and
     * invisible to untranslated-literals.spec.ts because an interpolation is not a literal.
     *
     * <p><b>These strings are web's, verbatim</b>, from `healthConnect.onboarding.verification.*`.
     * Not a style preference: a clinician reads this badge on the phone while a reviewer reads the
     * same document's status in the portal, so two wordings for one status is two people describing
     * one document differently. If a word here looks wrong — es REJECTED is `Rechazada`, feminine
     * against a masculine "documento" — fix it in `web/` and copy the fix back rather than diverging.
     */
    verification: {
      PENDING: 'Pending review',
      VERIFIED: 'Verified',
      REJECTED: 'Rejected',
    },
    /**
     * What to call a document whose type this build does not know — either because the server sent
     * a value added after this release, or because the row carries no type at all.
     *
     * <p>Deliberately outside `documentTypes` below, which holds the server's enum and only that:
     * `document-type-names.spec.ts` checks both directions over `DOCUMENT_TYPES`, so a tenth key in
     * that group would have to be excused by name. It is deliberately vague, too — it must not
     * claim a type the row does not have, which is why it is not `OTHER`'s word.
     */
    unknownType: 'Document',
    /**
     * The document type on the row title and in the upload picker, both of which rendered
     * `type.toLowerCase()` until backlog item 58 — `license`, `certificate`, `nhis`, in English, on
     * all four locales. Invisible to `untranslated-literals.spec.ts` for the same reason the status
     * badge was: an interpolation of a model value is neither visible text nor an attribute.
     *
     * <p><b>These strings are web's, verbatim</b>, from `healthConnect.onboarding.documentTypes.*`,
     * under the rule item 45 settled and CLAUDE.md states — a clinician reads this on the phone
     * while a reviewer reads the same document in the portal, so one credential must have one name.
     * A wording that looks wrong is fixed in `web/` and copied back, never diverged from here.
     */
    documentTypes: {
      CERTIFICATE: 'Professional certificate',
      LICENSE: 'License',
      PASSPORT: 'Passport',
      GHANACARD: 'Ghana Card',
      DRIVERLICENSE: "Driver's license",
      VOTERCARD: 'Voter card',
      PASSPHOTO: 'Passport photo',
      NHIS: 'NHIS card',
      OTHER: 'Other',
    },
  },
  tabs: {
    today: 'Today',
    messages: 'Messages',
    documents: 'Documents',
    me: 'Me',
  },
  /** The roster calendar, its day view, and own leave (Phase 4). */
  /** The patient directory and one patient's record (Phase 5). Read-only on the phone. */
  dashboard: {
    title: 'Dashboard',
    open: 'Dashboard',
    savedData: 'Showing saved figures',
    savedDataOffline: 'Offline — showing saved figures',
    patients: 'Patients',
    allPatients: 'All patients',
    children: 'Children',
    female: 'Female',
    male: 'Male',
    cases: 'Cases',
    openCases: 'Open',
    urgentCases: 'Urgent',
    closedCases: 'Closed',
    casesUnavailable: 'Case figures are unavailable right now. A dash means not known, not none.',
    noCharts: 'Charts and earnings are on the web portal.',
  },
  cases: {
    title: 'Cases',
    open: 'My cases',
    savedData: 'Showing saved cases',
    savedDataOffline: 'Offline — showing saved cases',
    empty: 'No cases in your queue.',
    loadFailed: 'Could not load your cases.',
    filterAll: 'All',
    filterOpen: 'Open',
    filterUrgent: 'Urgent',
    filterClosed: 'Closed',
    countOpen: 'Open',
    countUrgent: 'Urgent',
    countClosed: 'Closed',
    noBrief: 'Untitled case',
    loadingMore: 'Loading more',
    detailFailed: 'Could not open this case.',
    symptoms: 'Symptoms',
    diagnosis: 'Diagnosis',
    opened: 'Opened',
    detail: 'Case',
    brief: 'Summary',
    status: 'Status',
    save: 'Save changes',
    saveQueued: 'Saved on this phone. It will be sent when you have signal.',
    noPermission: 'Your role cannot edit cases.',
    archive: 'Archive this case',
    archiveReason: 'Why are you archiving it?',
    archiveHint: 'The case stays in the patient record. It leaves your queue once this is sent.',
    archivePending: 'Archiving',
    archiveDoctorOnly: 'Only a doctor can archive a case.',
    archivedOn: 'Archived',
    archivedNote: 'This case is retired.',
    close: 'Close',
  },
  patients: {
    title: 'Patients',
    open: 'My patients',
    search: 'Search by name or id',
    savedData: 'Showing saved patients',
    savedDataOffline: 'Offline — showing saved patients',
    empty: 'No patients match.',
    emptyNone: 'You have no patients yet.',
    loadFailed: 'Could not load your patients.',
    filterAll: 'All',
    filterFemale: 'Female',
    filterMale: 'Male',
    filterChildren: 'Children',
    child: 'Child',
    lastSeen: 'Last seen',
    neverSeen: 'No activity recorded',
    recencyRestricted: 'Your role cannot see recent visits',
    rowsRestricted: 'Some patients are not listed: your role cannot read case assignments.',
    /**
     * The directory's third refusal, and the only one about what a <b>tap</b> will do.
     *
     * <p>`X-Restricted-Follow-Ups: record` says the record behind every row here will refuse
     * (`../docs/backlog.md` items 128 and 132). That is a different piece of news from either
     * sentence above — one blanks a column, the other says people are missing — and item 129's trap
     * has now caught three items in a row, so `restricted-parts.spec.ts` fails if any two of the
     * four converge in any locale.
     *
     * <p>Two sentences, in the shape `dashboard.casesUnavailable` and `activityRestricted` use: the
     * first names the rule and its consequence, the second pre-empts the wrong reading. Here the
     * wrong reading is *the phone is stuck* — a tap that does nothing on a touch device invites a
     * retry, which is exactly what the 503's own "may clear on retry" wording invites (item 113).
     *
     * <p>The noun for the record itself comes from `record` below, which is the anchor term — see
     * the comment there for why that is written down rather than left to taste.
     */
    recordsRestricted: 'Your role cannot open patient records, so these rows do not open. Retrying will not change it.',
    loadingMore: 'Loading more',
    /**
     * <b>The anchor term for a patient's clinical record in this app.</b> A new sentence about a
     * record takes its noun from here in each locale — `Expediente`, `Dossier`, `Akte` — rather
     * than reaching for a synonym. Naming the record's noun is not enough, though, because Spanish
     * has three nouns in this neighbourhood and each is <b>reserved</b> (`../docs/backlog.md`
     * item 147):
     *
     * <ul>
     *   <li><b>`expediente`</b> — the clinical record, and nothing else.
     *   <li><b>`historial`</b> — the <i>activity log</i> only (`historial de actividad`, as in
     *       `activityRestricted` below), never the record. The two refusals render side by side in
     *       the same restriction block, so one noun doing both jobs left a clinician told "su rol
     *       no puede ver el historial" unable to tell whether they had lost the record or the log.
     *   <li><b>`expediente de acreditación`</b> — a clinician's own credentialing dossier
     *       (`web/`'s onboarding copy), a third sense kept distinct by its qualifier.
     * </ul>
     *
     * <p>For German the record is `Patientenakte`/`Akte`, the clinical word — never `Datensatz`, a
     * database word, which is what `web/` wrote and item 132 removed.
     *
     * <p>For French the record is <b>`dossier`</b> and the <i>clinical case</i> is <b>`cas`</b> —
     * never `dossier`, which is <b>reserved for the record</b> (`../docs/backlog.md` item 175,
     * matching what `web/`'s dashboard and case-queue fr already said). This catalogue used one
     * noun for both senses, and the collision was visible in a single sentence:
     * `cases.archiveHint` read <i>"Le dossier reste dans le dossier du patient"</i> — the same
     * word twice for two different things — until the case sense moved to `cas`. An earlier
     * version of this paragraph said French was `dossier` everywhere and not in question; that was
     * true of the record noun and false of the pair, which is how the collision survived item
     * 147's own review.
     *
     * <p>Worth stating because it is what went wrong <i>across</i> the two clients on item 132,
     * and then <i>within</i> this catalogue on item 147: this key itself said `Historial` in
     * Spanish while `cannotFile`, in the same block, already said `expediente`. Neither reads as
     * an error alone, and <b>no gate in the estate can see either</b> — a synonym is not a missing
     * key, and no check compares nouns across repositories. The only person who ever notices is
     * the clinician who reads the portal and the phone in the same language, and who has no reason
     * to think the two words name one thing. The point of this comment is that the next person
     * adding a record sentence — or a log sentence — should not have to settle the nouns again.
     *
     * <p><b>Two terms in this catalogue are pinned rather than chosen, and they are written down
     * elsewhere:</b> `src/app/core/brand/brand-name.spec.ts` holds the brand word
     * <b>`Professional`</b> on the sign-in lockup (`../docs/backlog.md` item 185) and
     * <b>`Ghana Card`</b>, the official name of Ghana's national identity card (item 186). Each is
     * asserted as an exact string in every locale, so those two stay <i>untranslated</i> — where
     * the nouns above are a choice <i>between</i> translations. (That file guards a third key,
     * `boot.unlockTitle`, by containment and prohibition rather than exact match, for the
     * brand-name rule its header states.) That is the difference to keep in mind when adding a
     * string: the nouns settled here bind nothing but this comment, because <b>no gate can see a
     * synonym</b>, whereas the two pins are gate-enforced and breaking one fails a test whose name
     * carries its item number.
     */
    record: 'Record',
    recordFailed: 'Could not load this record.',
    recordOffline: 'Showing the copy saved on this phone.',
    contact: 'Contact',
    dateOfBirth: 'Date of birth',
    phone: 'Phone',
    email: 'Email',
    emergencyContact: 'Emergency contact',
    cases: 'Cases',
    noCases: 'No open cases.',
    activity: 'Activity',
    noActivity: 'No activity recorded.',
    /**
     * The record's own refusal — deliberately NOT `recencyRestricted`, which is the directory's.
     *
     * <p>One wire token, `lastActivity`, costs two different things (`../docs/backlog.md` item 129).
     * On the list it blanks a column, so the row says recent visits cannot be seen. Here the whole
     * panel is withheld, so saying the same thing would tell a pharmacist that sorting by recency is
     * unavailable while the patient's entire history is missing.
     *
     * <p>Two sentences on purpose, in the shape `dashboard.casesUnavailable` uses: the first names
     * the refusal, the second says what the blank does <b>not</b> mean. It must not claim entries
     * exist — nothing here has read them — only that this is not evidence that none do.
     */
    activityRestricted: "Your role cannot read this patient's activity history. Entries may exist that are not shown here.",
    medications: 'Medications',
    noMedications: 'No medications recorded.',
    reports: 'Reports',
    noReports: 'No reports filed.',
    cannotFile: 'Your role cannot file notes on a record.',
    fileActivity: 'Add an entry',
    fileReport: 'Add a report',
    activityTitle: 'What happened',
    activityDetail: 'Details',
    reportName: 'Report name',
    reportType: 'Type',
    file: 'Add',
    fileQueued: 'Saved on this phone. It will be sent when you have signal.',
    fileNeedsText: 'Write what happened before adding it.',
    noPermission: 'Your role cannot file notes.',
    close: 'Close',
  },
  /**
   * Shift names, one key per value of `DUTY_ROSTER_SHIFTS` in `core/api/duty-roster-api.service`.
   *
   * Both screens that show a shift rendered `{{ round.shift | titlecase }}` until 2026-09-04, which
   * put the raw `DAY`/`OFF` on the card in title case — acceptable-looking English and nothing
   * sensible in the other three languages, which is precisely why nobody noticed. `shift-names.spec`
   * derives the expected keys from that array, so a value added there fails here in all four
   * languages rather than reaching a screen.
   *
   * Bare names, unlike `web/`'s window-bearing ones: both screens already render `windowText(shift)`
   * beside the name, so a label carrying the hours would print them twice.
   */
  shiftType: {
    DAY: 'Day',
    EVENING: 'Evening',
    NIGHT: 'Night',
    OFF: 'Off',
    FLEXIBLE: 'Flexible',
  },
  roster: {
    title: 'Roster',
    open: 'Open my roster',
    savedData: 'Showing saved data',
    savedDataOffline: 'Offline — showing saved data',
    worked: 'On duty',
    onLeave: 'On leave',
    noneThisYear: 'Nothing rostered this year.',
    loadFailed: 'Could not load your roster.',
    dayTitle: 'Rounds',
    dayLoadFailed: 'Could not load this day. It needs a connection.',
    dayEmpty: 'Nothing rostered on this day.',
    visits: 'Visits',
    noVisits: 'No visits listed',
    unnamedCustomer: 'Details unavailable',
    close: 'Close',
    previousYear: 'Previous year',
    nextYear: 'Next year',
  },
  absence: {
    title: 'Time off',
    none: 'No time off booked.',
    request: 'Request time off',
    from: 'First day',
    to: 'Last day',
    type: 'Reason',
    typeHoliday: 'Holiday',
    typeSick: 'Sick',
    typeOther: 'Other',
    statusRequested: 'Requested',
    statusApproved: 'Approved',
    submit: 'Send request',
    withdraw: 'Withdraw',
    withdrawConfirm: 'Withdraw this time off?',
    requestFailed: 'Could not send your request.',
    withdrawFailed: 'Could not withdraw it.',
    datesInvalid: 'The last day cannot be before the first.',
    willSendLater: 'You are offline. This will be sent when you have signal.',
  },
  me: {
    birthDate: 'Date of birth',
    sex: 'Sex',
    cardType: 'ID type',
    cardNumber: 'ID number',
    address: 'Address',
    street: 'Street',
    city: 'City',
    region: 'Region',
    country: 'Country',
    nextOfKin: 'Next of kin',
    kinName: 'Name',
    kinRelationship: 'Relationship',
    kinPhone: 'Phone',
    password: 'Password',
    currentPassword: 'Current password',
    newPassword: 'New password',
    confirmPassword: 'Confirm new password',
    changePassword: 'Change password',
    changePasswordDetail: 'You stay signed in on this device after changing it.',
    passwordMismatch: 'The two new passwords do not match.',
    passwordLength: 'A password must be between 4 and 100 characters.',
    passwordChanged: 'Password changed.',
    passwordWrong: 'That current password was not right.',
    completion: {
      title: 'Profile completion',
      complete: 'Everything is in. An administrator reviews your credentials next.',
      incomplete: 'Your profile becomes active once these are complete and an administrator has vetted them.',
      requirements: {
        consent: 'Consent',
        profile: 'Personal details',
        address: 'Address',
        nextOfKin: 'Next of kin',
        certificate: 'Certificate',
        license: 'Licence',
        identity: 'Identity document',
        photo: 'Passport photo',
      },
    },
    title: 'Me',
    profile: 'Profile',
    firstName: 'First name',
    lastName: 'Last name',
    email: 'Email',
    phone: 'Mobile phone',
    saved: 'Profile saved',
    saveFailed: 'Could not save your profile',
    loadFailed: 'Could not load your profile',
    notifications: 'Notifications',
    pushMessages: 'New messages',
    pushCompliance: 'Credential reminders',
    pushSenderName: 'Show who sent it',
    pushSenderNameDetail: 'Notification previews are visible to anyone holding your phone, so names are hidden unless you turn this on.',
    prefsSaveFailed: 'Could not save your notification settings',
    language: 'Language',
    shareRoster: 'Share my roster',
    shareUnavailable: 'Sharing is not available on this device',
    noRoster: 'No upcoming assignments to share',
    rosterTitle: 'My duty roster',
    unsent: 'Not sent',
    unsentDetail: '{{count}} entries have not been sent yet.',
    unsentRetry: 'Try again',
    unsentDiscard: 'Discard',
    unsentConflict: 'Someone else changed this. Open it and re-apply your edit.',
    unsentRejected: 'The server refused this.',
    unsentExpired: 'This has been waiting over a day and will not be sent.',
    signOutBlocked: 'You have unsent entries',
    signOutBlockedDetail: 'Signing out erases them from this phone. Send them first, or discard them.',
    signOutSendNow: 'Send now',
    signOutDiscardAndLeave: 'Discard and sign out',
    signOut: 'Sign out',
    signOutConfirm: 'Sign out of this device?',
    signOutDetail: 'Your cached patient data and messages will be erased from this phone.',
  },
};

export const ES: typeof EN = {
  common: {
    save: 'Guardar',
    cancel: 'Cancelar',
    retry: 'Reintentar',
    loading: 'Cargando…',
    offline: 'Está sin conexión',
    ageNever: 'nunca',
    ageJustNow: 'ahora mismo',
    ageMinutes: 'hace {{count}} min',
    ageHours: 'hace {{count}} h',
    ageDays: 'hace {{count}} d',
    pendingUnsent: 'Aún sin enviar',
    pendingConflict: 'Modificado en otro sitio',
    pendingRejected: 'Rechazado',
    pendingExpired: 'Se ha desistido',
  },
  auth: {
    forgotPassword: '¿Ha olvidado su contraseña?',
    resetTitle: 'Restablecer su contraseña',
    resetIntro: 'Introduzca su correo electrónico y le enviaremos una clave de restablecimiento.',
    email: 'Correo electrónico',
    resetSend: 'Enviar la clave',
    resetFinishTitle: '¿Ya tiene una clave?',
    resetFinishIntro: 'El correo se abre en su navegador, así que copie la clave y péguela aquí.',
    resetKey: 'Clave de restablecimiento',
    newPassword: 'Nueva contraseña',
    resetFinish: 'Establecer la nueva contraseña',
    resetClose: 'Volver al inicio de sesión',
    resetNeedsEmail: 'Introduzca su correo electrónico.',
    resetRequested: 'Si esa dirección está registrada, la clave está en camino.',
    resetNeedsKey: 'Pegue la clave del correo.',
    passwordLength: 'La contraseña debe tener entre 4 y 100 caracteres.',
    resetDone: 'Su contraseña ha sido cambiada. Ya puede iniciar sesión con ella.',
    resetBadKey: 'No se reconoció esa clave. Puede haber caducado; solicite una nueva.',
    subtitle: 'Professional',
    username: 'Usuario',
    password: 'Contraseña',
    signIn: 'Iniciar sesión',
    noScreenLock:
      'Este dispositivo no tiene bloqueo de pantalla, por lo que Abofonsa BridgeCare le pedirá la contraseña cada vez que lo abra.',
    sessionExpired: 'Su sesión ha finalizado. Vuelva a iniciar sesión.',
    biometryChanged: 'La configuración biométrica de este dispositivo ha cambiado, por lo que debe volver a iniciar sesión.',
    badCredentials: 'El usuario y la contraseña no coinciden.',
    unreachable: 'No se pudo conectar con Abofonsa BridgeCare. Compruebe su conexión e inténtelo de nuevo.',
  },
  boot: {
    restoring: 'Restaurando su sesión',
    offline: 'No se pudo conectar con Abofonsa BridgeCare. Su sesión sigue siendo válida.',
    tryAgain: 'Intentar de nuevo',
    signInWithPassword: 'Iniciar sesión con contraseña',
    unlockPrompt: 'Desbloquee para continuar',
    unlock: 'Desbloquear',
    unlockReason: 'Desbloquear Abofonsa BridgeCare Professional',
    unlockTitle: 'Desbloquear Abofonsa Pro',
    usePassword: 'Usar contraseña',
  },
  today: {
    title: 'Hoy',
    savedData: 'Mostrando datos guardados',
    savedDataOffline: 'Sin conexión — mostrando datos guardados',
    updated: 'actualizado',
    finishApplication: 'Complete su solicitud',
    notActive: 'Su solicitud aún no está activa, por lo que no hay turnos asignados.',
    continueOnWeb: 'Continúe en professional.abofonsa.com para completarla.',
    needsAttention: 'Requiere atención',
    unreadMessages: 'Mensajes sin leer',
    licence: 'Licencia',
    expired: 'Caducado',
    expires: 'Caduca',
    lapsed: 'caducado',
    nextSevenDays: 'Próximos 7 días',
    rosterFailed: 'No se pudo cargar su turno.',
    nothingRostered: 'No hay turnos en los próximos 7 días.',
    noShiftAssigned: 'Sin turno asignado',
    onDuty: 'De servicio',
    nextShift: 'Próximo turno',
    nothingToday: 'Nada hoy',
    flexibleToday: 'Horario flexible hoy',
    nothingTodayNamed: 'Hoy no hay nada, {{name}}',
    until: 'Hasta las {{time}}',
    flexibleOn: 'Flexible el {{date}}',
    dateAtTime: '{{date}} a las {{time}}',
  },
  messages: {
    compose: 'Nuevo mensaje',
    toPeople: 'A personas',
    toRole: 'A un rol',
    selected: 'Seleccionado',
    searchRecipients: 'Buscar por nombre',
    searchHint: 'Escriba al menos dos letras para buscar.',
    subject: 'Asunto',
    body: 'Mensaje',
    composeSend: 'Enviar',
    composeQueued: 'Guardado en este teléfono. Se enviará cuando tenga cobertura.',
    composeNeedsBody: 'Escriba un mensaje antes de enviarlo.',
    composeNeedsRecipient: 'Elija al menos un destinatario o un rol.',
    composeRoleEmpty: 'Nadie tiene ese rol actualmente, por lo que este mensaje no llegaría a nadie.',
    composeFailed: 'No se pudo poner el mensaje en cola.',
    broadcastConfirm: '¿Enviar a todos los que tengan este rol?',
    broadcastCount: 'Llegará a {{count}} personas con el rol {{role}}.',
    title: 'Mensajes',
    savedData: 'Mostrando mensajes guardados',
    savedDataOffline: 'Sin conexión — mostrando mensajes guardados',
    noSubject: 'Sin asunto',
    loadFailed: 'No se pudieron cargar los mensajes.',
    empty: 'No hay mensajes.',
    close: 'Cerrar',
    threadEmpty: 'Aún no hay mensajes en esta conversación.',
    sendFailed: 'No se pudo enviar. Compruebe su conexión e inténtelo de nuevo.',
    conversation: 'Conversación',
    send: 'Enviar',
    me: 'Yo',
    replyPlaceholder: 'Escriba una respuesta',
  },
  documents: {
    title: 'Documentos',
    savedData: 'Mostrando documentos guardados',
    savedDataOffline: 'Sin conexión — mostrando documentos guardados',
    yours: 'Sus documentos',
    loadFailed: 'No se pudieron cargar sus documentos.',
    empty: 'Aún no hay documentos.',
    add: 'Añadir un documento',
    type: 'Tipo',
    choose: 'Elegir',
    label: 'Etiqueta',
    labelPlaceholder: '¿Qué documento es este?',
    expiryDate: 'Fecha de caducidad',
    takePhoto: 'Hacer una foto',
    choosePdfOrImage: 'Elegir un PDF o una imagen',
    privacyNote: 'Las fotos se convierten a JPEG y se eliminan los datos de ubicación antes de subirlas.',
    preparing: 'Preparando la imagen…',
    uploading: 'Subiendo…',
    uploaded: 'Subido',
    pendingReview: 'Aparecerá como pendiente hasta que un administrador lo verifique.',
    done: 'Hecho',
    labelRequired: 'Dé una etiqueta a este documento.',
    expiryRequired: 'Una licencia necesita su fecha de caducidad.',
    uploadPhotoTooLarge: 'Esta foto es demasiado grande incluso tras comprimirla. Inténtelo de nuevo con menos fondo en el encuadre.',
    uploadPhotoUnreadable: 'No se pudo leer esa imagen. Vuelva a hacer la foto o elija un PDF.',
    uploadTypeRejected: 'Solo se aceptan documentos PDF, PNG y JPEG.',
    uploadPdfTooLarge: 'Ese PDF supera los 5 MB. Pruebe con un escaneo más pequeño.',
    uploadImageTooLarge: 'Esa imagen es demasiado grande incluso tras comprimirla.',
    uploadImageUnreadable: 'No se pudo leer esa imagen.',
    uploadFailed: 'Error al subir el archivo. Compruebe su conexión e inténtelo de nuevo.',
    addOrRenew: 'Añadir o renovar un documento',
    expires: 'Caduca',
    superseded: 'Reemplazado',
    verification: {
      PENDING: 'Pendiente de revisión',
      VERIFIED: 'Verificado',
      REJECTED: 'Rechazada',
    },
    unknownType: 'Documento',
    documentTypes: {
      CERTIFICATE: 'Certificado profesional',
      LICENSE: 'Licencia',
      PASSPORT: 'Pasaporte',
      GHANACARD: 'Ghana Card',
      DRIVERLICENSE: 'Permiso de conducir',
      VOTERCARD: 'Tarjeta de votante',
      PASSPHOTO: 'Foto de pasaporte',
      NHIS: 'Tarjeta NHIS',
      OTHER: 'Otro',
    },
  },
  tabs: {
    today: 'Hoy',
    messages: 'Mensajes',
    documents: 'Documentos',
    me: 'Yo',
  },
  dashboard: {
    title: 'Panel',
    open: 'Panel',
    savedData: 'Mostrando cifras guardadas',
    savedDataOffline: 'Sin conexión — mostrando cifras guardadas',
    patients: 'Pacientes',
    allPatients: 'Todos los pacientes',
    children: 'Niños',
    female: 'Mujeres',
    male: 'Hombres',
    cases: 'Casos',
    openCases: 'Abiertos',
    urgentCases: 'Urgentes',
    closedCases: 'Cerrados',
    casesUnavailable: 'Las cifras de casos no están disponibles ahora. Un guion significa que se desconoce, no que sea cero.',
    noCharts: 'Los gráficos y los ingresos están en el portal web.',
  },
  cases: {
    title: 'Casos',
    open: 'Mis casos',
    savedData: 'Mostrando casos guardados',
    savedDataOffline: 'Sin conexión — mostrando casos guardados',
    empty: 'No hay casos en su cola.',
    loadFailed: 'No se pudieron cargar sus casos.',
    filterAll: 'Todos',
    filterOpen: 'Abiertos',
    filterUrgent: 'Urgentes',
    filterClosed: 'Cerrados',
    countOpen: 'Abiertos',
    countUrgent: 'Urgentes',
    countClosed: 'Cerrados',
    noBrief: 'Caso sin título',
    loadingMore: 'Cargando más',
    detailFailed: 'No se pudo abrir este caso.',
    symptoms: 'Síntomas',
    diagnosis: 'Diagnóstico',
    opened: 'Abierto el',
    detail: 'Caso',
    brief: 'Resumen',
    status: 'Estado',
    save: 'Guardar cambios',
    saveQueued: 'Guardado en este teléfono. Se enviará cuando tenga cobertura.',
    noPermission: 'Su rol no puede editar casos.',
    archive: 'Archivar este caso',
    archiveReason: '¿Por qué lo archiva?',
    archiveHint: 'El caso permanece en el expediente del paciente. Saldrá de su lista cuando se envíe.',
    archivePending: 'Archivando',
    archiveDoctorOnly: 'Solo un médico puede archivar un caso.',
    archivedOn: 'Archivado el',
    archivedNote: 'Este caso ya no está activo.',
    close: 'Cerrar',
  },
  patients: {
    title: 'Pacientes',
    open: 'Mis pacientes',
    search: 'Buscar por nombre o identificador',
    savedData: 'Mostrando pacientes guardados',
    savedDataOffline: 'Sin conexión — mostrando pacientes guardados',
    empty: 'Ningún paciente coincide.',
    emptyNone: 'Todavía no tiene pacientes.',
    loadFailed: 'No se pudieron cargar sus pacientes.',
    filterAll: 'Todos',
    filterFemale: 'Mujeres',
    filterMale: 'Hombres',
    filterChildren: 'Menores',
    child: 'Menor',
    lastSeen: 'Última atención',
    neverSeen: 'Sin actividad registrada',
    recencyRestricted: 'Su rol no puede ver la actividad reciente',
    rowsRestricted: 'Algunos pacientes no aparecen: su rol no puede consultar las asignaciones de casos.',
    recordsRestricted: 'Su rol no puede abrir expedientes de pacientes, por lo que estas filas no se abren. Reintentar no lo cambiará.',
    loadingMore: 'Cargando más',
    record: 'Expediente',
    recordFailed: 'No se pudo cargar este expediente.',
    recordOffline: 'Mostrando la copia guardada en este teléfono.',
    contact: 'Contacto',
    dateOfBirth: 'Fecha de nacimiento',
    phone: 'Teléfono',
    email: 'Correo electrónico',
    emergencyContact: 'Contacto de emergencia',
    cases: 'Casos',
    noCases: 'No hay casos abiertos.',
    activity: 'Actividad',
    noActivity: 'Sin actividad registrada.',
    activityRestricted:
      'Su rol no puede consultar el historial de actividad de este paciente. Puede haber entradas que no se muestran aquí.',
    medications: 'Medicación',
    noMedications: 'Sin medicación registrada.',
    reports: 'Informes',
    noReports: 'No hay informes.',
    cannotFile: 'Su rol no puede registrar notas en un expediente.',
    fileActivity: 'Añadir una entrada',
    fileReport: 'Añadir un informe',
    activityTitle: 'Qué ha ocurrido',
    activityDetail: 'Detalles',
    reportName: 'Nombre del informe',
    reportType: 'Tipo',
    file: 'Añadir',
    fileQueued: 'Guardado en este teléfono. Se enviará cuando tenga cobertura.',
    fileNeedsText: 'Escriba qué ha ocurrido antes de añadirlo.',
    noPermission: 'Su rol no puede registrar notas.',
    close: 'Cerrar',
  },
  /**
   * Shift names, one key per value of `DUTY_ROSTER_SHIFTS` in `core/api/duty-roster-api.service`.
   *
   * Both screens that show a shift rendered `{{ round.shift | titlecase }}` until 2026-09-04, which
   * put the raw `DAY`/`OFF` on the card in title case — acceptable-looking English and nothing
   * sensible in the other three languages, which is precisely why nobody noticed. `shift-names.spec`
   * derives the expected keys from that array, so a value added there fails here in all four
   * languages rather than reaching a screen.
   *
   * Bare names, unlike `web/`'s window-bearing ones: both screens already render `windowText(shift)`
   * beside the name, so a label carrying the hours would print them twice.
   */
  shiftType: {
    DAY: 'Día',
    EVENING: 'Tarde',
    NIGHT: 'Noche',
    OFF: 'Libre',
    FLEXIBLE: 'Flexible',
  },
  roster: {
    title: 'Turnos',
    open: 'Ver mis turnos',
    savedData: 'Mostrando datos guardados',
    savedDataOffline: 'Sin conexión — mostrando datos guardados',
    worked: 'De servicio',
    onLeave: 'De permiso',
    noneThisYear: 'No hay nada asignado este año.',
    loadFailed: 'No se pudieron cargar sus turnos.',
    dayTitle: 'Rondas',
    dayLoadFailed: 'No se pudo cargar este día. Requiere conexión.',
    dayEmpty: 'No hay nada asignado este día.',
    visits: 'Visitas',
    noVisits: 'No hay visitas indicadas',
    unnamedCustomer: 'Datos no disponibles',
    close: 'Cerrar',
    previousYear: 'Año anterior',
    nextYear: 'Año siguiente',
  },
  absence: {
    title: 'Ausencias',
    none: 'No hay ausencias registradas.',
    request: 'Solicitar ausencia',
    from: 'Primer día',
    to: 'Último día',
    type: 'Motivo',
    typeHoliday: 'Vacaciones',
    typeSick: 'Enfermedad',
    typeOther: 'Otro',
    statusRequested: 'Solicitada',
    statusApproved: 'Aprobada',
    submit: 'Enviar solicitud',
    withdraw: 'Retirar',
    withdrawConfirm: '¿Retirar esta ausencia?',
    requestFailed: 'No se pudo enviar su solicitud.',
    withdrawFailed: 'No se pudo retirar.',
    datesInvalid: 'El último día no puede ser anterior al primero.',
    willSendLater: 'Está sin conexión. Esto se enviará cuando tenga cobertura.',
  },
  me: {
    birthDate: 'Fecha de nacimiento',
    sex: 'Sexo',
    cardType: 'Tipo de documento',
    cardNumber: 'Número de documento',
    address: 'Dirección',
    street: 'Calle',
    city: 'Ciudad',
    region: 'Región',
    country: 'País',
    nextOfKin: 'Contacto de emergencia',
    kinName: 'Nombre',
    kinRelationship: 'Parentesco',
    kinPhone: 'Teléfono',
    password: 'Contraseña',
    currentPassword: 'Contraseña actual',
    newPassword: 'Nueva contraseña',
    confirmPassword: 'Confirmar la nueva contraseña',
    changePassword: 'Cambiar contraseña',
    changePasswordDetail: 'Su sesión seguirá abierta en este dispositivo después del cambio.',
    passwordMismatch: 'Las dos contraseñas nuevas no coinciden.',
    passwordLength: 'La contraseña debe tener entre 4 y 100 caracteres.',
    passwordChanged: 'Contraseña cambiada.',
    passwordWrong: 'Esa contraseña actual no era correcta.',
    completion: {
      title: 'Perfil completado',
      complete: 'Todo está listo. Un administrador revisará sus credenciales a continuación.',
      incomplete: 'Su perfil se activará cuando estén completos y un administrador los haya verificado.',
      requirements: {
        consent: 'Consentimiento',
        profile: 'Datos personales',
        address: 'Dirección',
        nextOfKin: 'Contacto de emergencia',
        certificate: 'Certificado',
        license: 'Licencia',
        identity: 'Documento de identidad',
        photo: 'Fotografía',
      },
    },
    title: 'Yo',
    profile: 'Perfil',
    firstName: 'Nombre',
    lastName: 'Apellidos',
    email: 'Correo electrónico',
    phone: 'Teléfono móvil',
    saved: 'Perfil guardado',
    saveFailed: 'No se pudo guardar su perfil',
    loadFailed: 'No se pudo cargar su perfil',
    notifications: 'Notificaciones',
    pushMessages: 'Mensajes nuevos',
    pushCompliance: 'Avisos de acreditación',
    pushSenderName: 'Mostrar quién lo envía',
    pushSenderNameDetail:
      'Las vistas previas de las notificaciones son visibles para cualquiera que tenga su teléfono, por lo que los nombres se ocultan salvo que active esta opción.',
    prefsSaveFailed: 'No se pudieron guardar sus ajustes de notificaciones',
    language: 'Idioma',
    shareRoster: 'Compartir mi turno',
    shareUnavailable: 'Compartir no está disponible en este dispositivo',
    noRoster: 'No hay turnos próximos para compartir',
    rosterTitle: 'Mi turno de servicio',
    unsent: 'Sin enviar',
    unsentDetail: '{{count}} entradas aún no se han enviado.',
    unsentRetry: 'Reintentar',
    unsentDiscard: 'Descartar',
    unsentConflict: 'Otra persona lo ha modificado. Ábralo y vuelva a aplicar su cambio.',
    unsentRejected: 'El servidor lo ha rechazado.',
    unsentExpired: 'Lleva más de un día esperando y no se enviará.',
    signOutBlocked: 'Tiene entradas sin enviar',
    signOutBlockedDetail: 'Al cerrar sesión se borrarán de este teléfono. Envíelas primero o descártelas.',
    signOutSendNow: 'Enviar ahora',
    signOutDiscardAndLeave: 'Descartar y cerrar sesión',
    signOut: 'Cerrar sesión',
    signOutConfirm: '¿Cerrar sesión en este dispositivo?',
    signOutDetail: 'Los datos de pacientes y mensajes en caché se borrarán de este teléfono.',
  },
};

export const FR: typeof EN = {
  common: {
    save: 'Enregistrer',
    cancel: 'Annuler',
    retry: 'Réessayer',
    loading: 'Chargement…',
    offline: 'Vous êtes hors ligne',
    ageNever: 'jamais',
    ageJustNow: "à l'instant",
    ageMinutes: 'il y a {{count}} min',
    ageHours: 'il y a {{count}} h',
    ageDays: 'il y a {{count}} j',
    pendingUnsent: 'Pas encore envoyé',
    pendingConflict: 'Modifié ailleurs',
    pendingRejected: 'Refusé',
    pendingExpired: 'Abandonné',
  },
  auth: {
    forgotPassword: 'Mot de passe oublié ?',
    resetTitle: 'Réinitialiser votre mot de passe',
    resetIntro: 'Saisissez votre adresse e-mail et nous vous enverrons une clé de réinitialisation.',
    email: 'Adresse e-mail',
    resetSend: 'Envoyer la clé',
    resetFinishTitle: 'Vous avez déjà une clé ?',
    resetFinishIntro: "L'e-mail s'ouvre dans votre navigateur : copiez-y la clé et collez-la ici.",
    resetKey: 'Clé de réinitialisation',
    newPassword: 'Nouveau mot de passe',
    resetFinish: 'Définir le nouveau mot de passe',
    resetClose: 'Retour à la connexion',
    resetNeedsEmail: 'Saisissez votre adresse e-mail.',
    resetRequested: 'Si cette adresse est enregistrée, une clé est en route.',
    resetNeedsKey: 'Collez la clé reçue par e-mail.',
    passwordLength: 'Un mot de passe doit comporter entre 4 et 100 caractères.',
    resetDone: 'Votre mot de passe a été modifié. Vous pouvez maintenant vous connecter avec.',
    resetBadKey: "Cette clé n'a pas été reconnue. Elle a peut-être expiré — demandez-en une nouvelle.",
    subtitle: 'Professional',
    username: "Nom d'utilisateur",
    password: 'Mot de passe',
    signIn: 'Se connecter',
    noScreenLock:
      "Cet appareil n'a pas de verrouillage d'écran ; Abofonsa BridgeCare vous demandera donc votre mot de passe à chaque ouverture.",
    sessionExpired: 'Votre session a pris fin. Veuillez vous reconnecter.',
    biometryChanged: 'Les paramètres biométriques de cet appareil ont changé ; vous devez vous reconnecter.',
    badCredentials: "Ce nom d'utilisateur et ce mot de passe ne correspondent pas.",
    unreachable: 'Impossible de joindre Abofonsa BridgeCare. Vérifiez votre connexion et réessayez.',
  },
  boot: {
    restoring: 'Restauration de votre session',
    offline: 'Impossible de joindre Abofonsa BridgeCare. Votre session est toujours valide.',
    tryAgain: 'Réessayer',
    signInWithPassword: 'Se connecter avec un mot de passe',
    unlockPrompt: 'Déverrouillez pour continuer',
    unlock: 'Déverrouiller',
    unlockReason: 'Déverrouiller Abofonsa BridgeCare Professional',
    unlockTitle: 'Déverrouiller Abofonsa Pro',
    usePassword: 'Utiliser le mot de passe',
  },
  today: {
    title: "Aujourd'hui",
    savedData: 'Affichage des données enregistrées',
    savedDataOffline: 'Hors ligne — affichage des données enregistrées',
    updated: 'mis à jour',
    finishApplication: 'Terminez votre candidature',
    notActive: "Votre candidature n'est pas encore active, il n'y a donc aucune affectation.",
    continueOnWeb: 'Continuez sur professional.abofonsa.com pour la terminer.',
    needsAttention: 'À traiter',
    unreadMessages: 'Messages non lus',
    licence: "Autorisation d'exercice",
    expired: 'Expiré',
    expires: 'Expire le',
    lapsed: 'expiré',
    nextSevenDays: '7 prochains jours',
    rosterFailed: 'Impossible de charger votre planning.',
    nothingRostered: 'Aucune affectation dans les 7 prochains jours.',
    noShiftAssigned: 'Aucun service attribué',
    onDuty: 'En service',
    nextShift: 'Prochain service',
    nothingToday: "Rien aujourd'hui",
    flexibleToday: "Horaires flexibles aujourd'hui",
    nothingTodayNamed: "Rien aujourd'hui, {{name}}",
    until: "Jusqu'à {{time}}",
    flexibleOn: 'Flexible le {{date}}',
    dateAtTime: '{{date}} à {{time}}',
  },
  messages: {
    compose: 'Nouveau message',
    toPeople: 'À des personnes',
    toRole: 'À un rôle',
    selected: 'Sélectionné',
    searchRecipients: 'Rechercher par nom',
    searchHint: 'Saisissez au moins deux lettres pour rechercher.',
    subject: 'Objet',
    body: 'Message',
    composeSend: 'Envoyer',
    composeQueued: 'Enregistré sur ce téléphone. Il sera envoyé dès que vous aurez du réseau.',
    composeNeedsBody: "Écrivez un message avant de l'envoyer.",
    composeNeedsRecipient: 'Choisissez au moins un destinataire ou un rôle.',
    composeRoleEmpty: "Personne ne détient actuellement ce rôle, ce message n'atteindrait donc personne.",
    composeFailed: "Le message n'a pas pu être mis en file d'attente.",
    broadcastConfirm: 'Envoyer à toutes les personnes ayant ce rôle ?',
    broadcastCount: 'Ce message ira à {{count}} personnes ayant le rôle {{role}}.',
    title: 'Messages',
    savedData: 'Affichage des messages enregistrés',
    savedDataOffline: 'Hors ligne — affichage des messages enregistrés',
    noSubject: 'Sans objet',
    loadFailed: 'Impossible de charger les messages.',
    empty: 'Aucun message.',
    close: 'Fermer',
    threadEmpty: 'Aucun message dans cette conversation pour le moment.',
    sendFailed: "Impossible d'envoyer. Vérifiez votre connexion et réessayez.",
    conversation: 'Conversation',
    send: 'Envoyer',
    me: 'Moi',
    replyPlaceholder: 'Écrivez une réponse',
  },
  documents: {
    title: 'Documents',
    savedData: 'Affichage des documents enregistrés',
    savedDataOffline: 'Hors ligne — affichage des documents enregistrés',
    yours: 'Vos documents',
    loadFailed: 'Impossible de charger vos documents.',
    empty: 'Aucun document pour le moment.',
    add: 'Ajouter un document',
    type: 'Type',
    choose: 'Choisir',
    label: 'Libellé',
    labelPlaceholder: "De quel document s'agit-il ?",
    expiryDate: "Date d'expiration",
    takePhoto: 'Prendre une photo',
    choosePdfOrImage: 'Choisir un PDF ou une image',
    privacyNote: 'Les photos sont converties en JPEG et les données de localisation sont supprimées avant envoi.',
    preparing: "Préparation de l'image…",
    uploading: 'Envoi en cours…',
    uploaded: 'Envoyé',
    pendingReview: "Il apparaîtra comme en attente jusqu'à ce qu'un administrateur le vérifie.",
    done: 'Terminé',
    labelRequired: 'Donnez un libellé à ce document.',
    expiryRequired: "Une autorisation d'exercice doit avoir sa date d'expiration.",
    uploadPhotoTooLarge: "Cette photo est trop volumineuse même après compression. Réessayez avec moins d'arrière-plan dans le cadre.",
    uploadPhotoUnreadable: 'Impossible de lire cette image. Reprenez la photo ou choisissez un PDF.',
    uploadTypeRejected: 'Seuls les documents PDF, PNG et JPEG sont acceptés.',
    uploadPdfTooLarge: 'Ce PDF dépasse 5 Mo. Essayez avec un scan plus petit.',
    uploadImageTooLarge: 'Cette image est trop volumineuse même après compression.',
    uploadImageUnreadable: 'Impossible de lire cette image.',
    uploadFailed: "L'envoi a échoué. Vérifiez votre connexion et réessayez.",
    addOrRenew: 'Ajouter ou renouveler un document',
    expires: 'Expire le',
    superseded: 'Remplacé',
    verification: {
      PENDING: 'En attente',
      VERIFIED: 'Vérifié',
      REJECTED: 'Rejeté',
    },
    unknownType: 'Document',
    documentTypes: {
      CERTIFICATE: 'Certificat professionnel',
      LICENSE: "Autorisation d'exercice",
      PASSPORT: 'Passeport',
      GHANACARD: 'Ghana Card',
      DRIVERLICENSE: 'Permis de conduire',
      VOTERCARD: "Carte d'électeur",
      PASSPHOTO: "Photo d'identité",
      NHIS: 'Carte NHIS',
      OTHER: 'Autre',
    },
  },
  tabs: {
    today: "Aujourd'hui",
    messages: 'Messages',
    documents: 'Documents',
    me: 'Moi',
  },
  dashboard: {
    title: 'Tableau de bord',
    open: 'Tableau de bord',
    savedData: 'Affichage des chiffres enregistrés',
    savedDataOffline: 'Hors ligne — affichage des chiffres enregistrés',
    patients: 'Patients',
    allPatients: 'Tous les patients',
    children: 'Enfants',
    female: 'Femmes',
    male: 'Hommes',
    cases: 'Cas',
    openCases: 'Ouverts',
    urgentCases: 'Urgents',
    closedCases: 'Clos',
    casesUnavailable: 'Les chiffres des cas ne sont pas disponibles pour le moment. Un tiret signifie inconnu, pas zéro.',
    noCharts: 'Les graphiques et les revenus sont sur le portail web.',
  },
  cases: {
    title: 'Cas',
    open: 'Mes cas',
    savedData: 'Affichage des cas enregistrés',
    savedDataOffline: 'Hors ligne — affichage des cas enregistrés',
    empty: 'Aucun cas dans votre file.',
    loadFailed: 'Impossible de charger vos cas.',
    filterAll: 'Tous',
    filterOpen: 'Ouverts',
    filterUrgent: 'Urgents',
    filterClosed: 'Clos',
    countOpen: 'Ouverts',
    countUrgent: 'Urgents',
    countClosed: 'Clos',
    noBrief: 'Cas sans titre',
    loadingMore: 'Chargement',
    detailFailed: "Impossible d'ouvrir ce cas.",
    symptoms: 'Symptômes',
    diagnosis: 'Diagnostic',
    opened: 'Ouvert le',
    detail: 'Cas',
    brief: 'Résumé',
    status: 'Statut',
    save: 'Enregistrer',
    saveQueued: 'Enregistré sur ce téléphone. Il sera envoyé dès que vous aurez du réseau.',
    noPermission: 'Votre rôle ne permet pas de modifier les cas.',
    archive: 'Archiver ce cas',
    archiveReason: 'Pourquoi archivez-vous ce cas ?',
    archiveHint: "Le cas reste dans le dossier du patient. Il quitte votre liste une fois l'envoi effectué.",
    archivePending: 'Archivage',
    archiveDoctorOnly: 'Seul un médecin peut archiver un cas.',
    archivedOn: 'Archivé le',
    archivedNote: "Ce cas n'est plus actif.",
    close: 'Fermer',
  },
  patients: {
    title: 'Patients',
    open: 'Mes patients',
    search: 'Rechercher par nom ou identifiant',
    savedData: 'Affichage des patients enregistrés',
    savedDataOffline: 'Hors ligne — affichage des patients enregistrés',
    empty: 'Aucun patient ne correspond.',
    emptyNone: "Vous n'avez pas encore de patients.",
    loadFailed: 'Impossible de charger vos patients.',
    filterAll: 'Tous',
    filterFemale: 'Femmes',
    filterMale: 'Hommes',
    filterChildren: 'Enfants',
    child: 'Enfant',
    lastSeen: 'Dernier passage',
    neverSeen: 'Aucune activité enregistrée',
    recencyRestricted: 'Votre rôle ne permet pas de voir les passages récents',
    rowsRestricted: 'Certains patients ne sont pas affichés : votre rôle ne permet pas de consulter les affectations de cas.',
    recordsRestricted:
      "Votre rôle ne permet pas d'ouvrir les dossiers patients ; ces lignes ne s'ouvrent donc pas. Réessayer n'y changera rien.",
    loadingMore: 'Chargement',
    record: 'Dossier',
    recordFailed: 'Impossible de charger ce dossier.',
    recordOffline: 'Affichage de la copie enregistrée sur ce téléphone.',
    contact: 'Coordonnées',
    dateOfBirth: 'Date de naissance',
    phone: 'Téléphone',
    email: 'E-mail',
    emergencyContact: "Contact d'urgence",
    cases: 'Cas',
    noCases: 'Aucun cas ouvert.',
    activity: 'Activité',
    noActivity: 'Aucune activité enregistrée.',
    activityRestricted:
      "Votre rôle ne permet pas de consulter l'historique d'activité de ce patient. Des entrées peuvent exister sans être affichées ici.",
    medications: 'Médicaments',
    noMedications: 'Aucun médicament enregistré.',
    reports: 'Rapports',
    noReports: 'Aucun rapport.',
    cannotFile: "Votre rôle ne permet pas d'ajouter des notes à un dossier.",
    fileActivity: 'Ajouter une entrée',
    fileReport: 'Ajouter un rapport',
    activityTitle: "Ce qui s'est passé",
    activityDetail: 'Détails',
    reportName: 'Nom du rapport',
    reportType: 'Type',
    file: 'Ajouter',
    fileQueued: 'Enregistré sur ce téléphone. Il sera envoyé dès que vous aurez du réseau.',
    fileNeedsText: "Décrivez ce qui s'est passé avant d'ajouter.",
    noPermission: 'Votre rôle ne permet pas de saisir des notes.',
    close: 'Fermer',
  },
  /**
   * Shift names, one key per value of `DUTY_ROSTER_SHIFTS` in `core/api/duty-roster-api.service`.
   *
   * Both screens that show a shift rendered `{{ round.shift | titlecase }}` until 2026-09-04, which
   * put the raw `DAY`/`OFF` on the card in title case — acceptable-looking English and nothing
   * sensible in the other three languages, which is precisely why nobody noticed. `shift-names.spec`
   * derives the expected keys from that array, so a value added there fails here in all four
   * languages rather than reaching a screen.
   *
   * Bare names, unlike `web/`'s window-bearing ones: both screens already render `windowText(shift)`
   * beside the name, so a label carrying the hours would print them twice.
   */
  shiftType: {
    DAY: 'Journée',
    EVENING: 'Soirée',
    NIGHT: 'Nuit',
    OFF: 'Repos',
    FLEXIBLE: 'Flexible',
  },
  roster: {
    title: 'Planning',
    open: 'Voir mon planning',
    savedData: 'Affichage des données enregistrées',
    savedDataOffline: 'Hors ligne — affichage des données enregistrées',
    worked: 'De service',
    onLeave: 'En congé',
    noneThisYear: 'Rien de planifié cette année.',
    loadFailed: 'Impossible de charger votre planning.',
    dayTitle: 'Tournées',
    dayLoadFailed: 'Impossible de charger cette journée. Une connexion est nécessaire.',
    dayEmpty: 'Rien de planifié ce jour-là.',
    visits: 'Visites',
    noVisits: 'Aucune visite indiquée',
    unnamedCustomer: 'Coordonnées indisponibles',
    close: 'Fermer',
    previousYear: 'Année précédente',
    nextYear: 'Année suivante',
  },
  absence: {
    title: 'Congés',
    none: 'Aucun congé enregistré.',
    request: 'Demander un congé',
    from: 'Premier jour',
    to: 'Dernier jour',
    type: 'Motif',
    typeHoliday: 'Vacances',
    typeSick: 'Maladie',
    typeOther: 'Autre',
    statusRequested: 'Demandé',
    statusApproved: 'Approuvé',
    submit: 'Envoyer la demande',
    withdraw: 'Retirer',
    withdrawConfirm: 'Retirer ce congé ?',
    requestFailed: "Impossible d'envoyer votre demande.",
    withdrawFailed: 'Impossible de le retirer.',
    datesInvalid: 'Le dernier jour ne peut pas précéder le premier.',
    willSendLater: 'Vous êtes hors ligne. Ceci sera envoyé dès que vous aurez du réseau.',
  },
  me: {
    birthDate: 'Date de naissance',
    sex: 'Sexe',
    cardType: 'Type de pièce',
    cardNumber: 'Numéro de pièce',
    address: 'Adresse',
    street: 'Rue',
    city: 'Ville',
    region: 'Région',
    country: 'Pays',
    nextOfKin: 'Personne à prévenir',
    kinName: 'Nom',
    kinRelationship: 'Lien',
    kinPhone: 'Téléphone',
    password: 'Mot de passe',
    currentPassword: 'Mot de passe actuel',
    newPassword: 'Nouveau mot de passe',
    confirmPassword: 'Confirmer le nouveau mot de passe',
    changePassword: 'Changer le mot de passe',
    changePasswordDetail: 'Vous restez connecté sur cet appareil après le changement.',
    passwordMismatch: 'Les deux nouveaux mots de passe ne correspondent pas.',
    passwordLength: 'Un mot de passe doit comporter entre 4 et 100 caractères.',
    passwordChanged: 'Mot de passe modifié.',
    passwordWrong: "Ce mot de passe actuel n'était pas correct.",
    completion: {
      title: 'Profil complété',
      complete: 'Tout est là. Un administrateur va maintenant vérifier vos justificatifs.',
      incomplete: 'Votre profil deviendra actif une fois ces éléments complets et vérifiés par un administrateur.',
      requirements: {
        consent: 'Consentement',
        profile: 'Informations personnelles',
        address: 'Adresse',
        nextOfKin: 'Personne à prévenir',
        certificate: 'Certificat',
        license: "Autorisation d'exercice",
        identity: "Pièce d'identité",
        photo: 'Photo',
      },
    },
    title: 'Moi',
    profile: 'Profil',
    firstName: 'Prénom',
    lastName: 'Nom',
    email: 'E-mail',
    phone: 'Téléphone mobile',
    saved: 'Profil enregistré',
    saveFailed: "Impossible d'enregistrer votre profil",
    loadFailed: 'Impossible de charger votre profil',
    notifications: 'Notifications',
    pushMessages: 'Nouveaux messages',
    pushCompliance: 'Rappels de justificatifs',
    pushSenderName: "Afficher l'expéditeur",
    pushSenderNameDetail:
      "Les aperçus de notification sont visibles par toute personne qui tient votre téléphone : les noms restent masqués tant que vous n'activez pas cette option.",
    prefsSaveFailed: "Impossible d'enregistrer vos paramètres de notification",
    language: 'Langue',
    shareRoster: 'Partager mon planning',
    shareUnavailable: "Le partage n'est pas disponible sur cet appareil",
    noRoster: 'Aucune affectation à venir à partager',
    rosterTitle: 'Mon planning de garde',
    unsent: 'Non envoyé',
    unsentDetail: "{{count}} entrées n'ont pas encore été envoyées.",
    unsentRetry: 'Réessayer',
    unsentDiscard: 'Supprimer',
    unsentConflict: "Quelqu'un d'autre a modifié ceci. Ouvrez-le et réappliquez votre modification.",
    unsentRejected: 'Le serveur a refusé cet envoi.',
    unsentExpired: "Cela attend depuis plus d'un jour et ne sera pas envoyé.",
    signOutBlocked: 'Vous avez des entrées non envoyées',
    signOutBlockedDetail: 'La déconnexion les effacera de ce téléphone. Envoyez-les ou supprimez-les.',
    signOutSendNow: 'Envoyer maintenant',
    signOutDiscardAndLeave: 'Supprimer et se déconnecter',
    signOut: 'Se déconnecter',
    signOutConfirm: 'Se déconnecter de cet appareil ?',
    signOutDetail: 'Les données patients et les messages en cache seront effacés de ce téléphone.',
  },
};

export const DE: typeof EN = {
  common: {
    save: 'Speichern',
    cancel: 'Abbrechen',
    retry: 'Erneut versuchen',
    loading: 'Wird geladen…',
    offline: 'Sie sind offline',
    ageNever: 'nie',
    ageJustNow: 'gerade eben',
    ageMinutes: 'vor {{count}} Min.',
    ageHours: 'vor {{count}} Std.',
    ageDays: 'vor {{count}} T.',
    pendingUnsent: 'Noch nicht gesendet',
    pendingConflict: 'Anderswo geändert',
    pendingRejected: 'Abgelehnt',
    pendingExpired: 'Aufgegeben',
  },
  auth: {
    forgotPassword: 'Passwort vergessen?',
    resetTitle: 'Passwort zurücksetzen',
    resetIntro: 'Geben Sie Ihre E-Mail-Adresse ein und wir senden Ihnen einen Schlüssel zum Zurücksetzen.',
    email: 'E-Mail-Adresse',
    resetSend: 'Schlüssel senden',
    resetFinishTitle: 'Sie haben bereits einen Schlüssel?',
    resetFinishIntro: 'Die E-Mail öffnet sich im Browser — kopieren Sie den Schlüssel und fügen Sie ihn hier ein.',
    resetKey: 'Schlüssel',
    newPassword: 'Neues Passwort',
    resetFinish: 'Neues Passwort festlegen',
    resetClose: 'Zurück zur Anmeldung',
    resetNeedsEmail: 'Geben Sie Ihre E-Mail-Adresse ein.',
    resetRequested: 'Falls diese Adresse registriert ist, ist ein Schlüssel unterwegs.',
    resetNeedsKey: 'Fügen Sie den Schlüssel aus der E-Mail ein.',
    passwordLength: 'Ein Passwort muss zwischen 4 und 100 Zeichen lang sein.',
    resetDone: 'Ihr Passwort wurde geändert. Sie können sich nun damit anmelden.',
    resetBadKey: 'Dieser Schlüssel wurde nicht erkannt. Er ist möglicherweise abgelaufen — fordern Sie einen neuen an.',
    subtitle: 'Professional',
    username: 'Benutzername',
    password: 'Passwort',
    signIn: 'Anmelden',
    noScreenLock: 'Dieses Gerät hat keine Bildschirmsperre. Abofonsa BridgeCare fragt daher bei jedem Öffnen nach Ihrem Passwort.',
    sessionExpired: 'Ihre Sitzung ist beendet. Bitte melden Sie sich erneut an.',
    biometryChanged: 'Die biometrischen Einstellungen dieses Geräts haben sich geändert. Sie müssen sich erneut anmelden.',
    badCredentials: 'Benutzername und Passwort stimmen nicht überein.',
    unreachable: 'Abofonsa BridgeCare ist nicht erreichbar. Prüfen Sie Ihre Verbindung und versuchen Sie es erneut.',
  },
  boot: {
    restoring: 'Ihre Sitzung wird wiederhergestellt',
    offline: 'Abofonsa BridgeCare ist nicht erreichbar. Ihre Sitzung ist weiterhin gültig.',
    tryAgain: 'Erneut versuchen',
    signInWithPassword: 'Mit Passwort anmelden',
    unlockPrompt: 'Zum Fortfahren entsperren',
    unlock: 'Entsperren',
    unlockReason: 'Abofonsa BridgeCare Professional entsperren',
    unlockTitle: 'Abofonsa Pro entsperren',
    usePassword: 'Passwort verwenden',
  },
  today: {
    title: 'Heute',
    savedData: 'Gespeicherte Daten werden angezeigt',
    savedDataOffline: 'Offline — gespeicherte Daten werden angezeigt',
    updated: 'aktualisiert',
    finishApplication: 'Bewerbung abschließen',
    notActive: 'Ihre Bewerbung ist noch nicht aktiv, daher sind keine Dienste eingeteilt.',
    continueOnWeb: 'Fahren Sie auf professional.abofonsa.com fort, um sie abzuschließen.',
    needsAttention: 'Erfordert Aufmerksamkeit',
    unreadMessages: 'Ungelesene Nachrichten',
    licence: 'Lizenz',
    expired: 'Abgelaufen',
    expires: 'Läuft ab am',
    lapsed: 'abgelaufen',
    nextSevenDays: 'Nächste 7 Tage',
    rosterFailed: 'Ihr Dienstplan konnte nicht geladen werden.',
    nothingRostered: 'Keine Dienste in den nächsten 7 Tagen.',
    noShiftAssigned: 'Kein Dienst zugewiesen',
    onDuty: 'Im Dienst',
    nextShift: 'Nächster Dienst',
    nothingToday: 'Heute nichts',
    flexibleToday: 'Heute flexible Arbeitszeit',
    nothingTodayNamed: 'Heute nichts, {{name}}',
    until: 'Bis {{time}}',
    flexibleOn: 'Flexibel am {{date}}',
    dateAtTime: '{{date}} um {{time}}',
  },
  messages: {
    compose: 'Neue Nachricht',
    toPeople: 'An Personen',
    toRole: 'An eine Rolle',
    selected: 'Ausgewählt',
    searchRecipients: 'Nach Namen suchen',
    searchHint: 'Geben Sie mindestens zwei Buchstaben ein, um zu suchen.',
    subject: 'Betreff',
    body: 'Nachricht',
    composeSend: 'Senden',
    composeQueued: 'Auf diesem Telefon gespeichert. Es wird gesendet, sobald Sie Empfang haben.',
    composeNeedsBody: 'Schreiben Sie eine Nachricht, bevor Sie sie senden.',
    composeNeedsRecipient: 'Wählen Sie mindestens einen Empfänger oder eine Rolle.',
    composeRoleEmpty: 'Diese Rolle hat derzeit niemand inne, diese Nachricht würde also niemanden erreichen.',
    composeFailed: 'Die Nachricht konnte nicht eingereiht werden.',
    broadcastConfirm: 'An alle mit dieser Rolle senden?',
    broadcastCount: 'Sie geht an {{count}} Personen mit der Rolle {{role}}.',
    title: 'Nachrichten',
    savedData: 'Gespeicherte Nachrichten werden angezeigt',
    savedDataOffline: 'Offline — gespeicherte Nachrichten werden angezeigt',
    noSubject: 'Kein Betreff',
    loadFailed: 'Nachrichten konnten nicht geladen werden.',
    empty: 'Keine Nachrichten.',
    close: 'Schließen',
    threadEmpty: 'Noch keine Nachrichten in diesem Verlauf.',
    sendFailed: 'Senden fehlgeschlagen. Prüfen Sie Ihre Verbindung und versuchen Sie es erneut.',
    conversation: 'Unterhaltung',
    send: 'Senden',
    me: 'Ich',
    replyPlaceholder: 'Antwort schreiben',
  },
  documents: {
    title: 'Dokumente',
    savedData: 'Gespeicherte Dokumente werden angezeigt',
    savedDataOffline: 'Offline — gespeicherte Dokumente werden angezeigt',
    yours: 'Ihre Dokumente',
    loadFailed: 'Ihre Dokumente konnten nicht geladen werden.',
    empty: 'Noch keine Dokumente.',
    add: 'Dokument hinzufügen',
    type: 'Typ',
    choose: 'Auswählen',
    label: 'Bezeichnung',
    labelPlaceholder: 'Um welches Dokument handelt es sich?',
    expiryDate: 'Ablaufdatum',
    takePhoto: 'Foto aufnehmen',
    choosePdfOrImage: 'PDF oder Bild auswählen',
    privacyNote: 'Fotos werden in JPEG umgewandelt und Standortdaten vor dem Hochladen entfernt.',
    preparing: 'Bild wird vorbereitet…',
    uploading: 'Wird hochgeladen…',
    uploaded: 'Hochgeladen',
    pendingReview: 'Es wird als ausstehend angezeigt, bis ein Administrator es überprüft.',
    done: 'Fertig',
    labelRequired: 'Geben Sie diesem Dokument eine Bezeichnung.',
    expiryRequired: 'Eine Lizenz benötigt ihr Ablaufdatum.',
    uploadPhotoTooLarge: 'Dieses Foto ist auch nach der Komprimierung zu groß. Versuchen Sie es erneut mit weniger Hintergrund im Bild.',
    uploadPhotoUnreadable: 'Dieses Bild konnte nicht gelesen werden. Fotografieren Sie erneut oder wählen Sie ein PDF.',
    uploadTypeRejected: 'Es werden nur PDF-, PNG- und JPEG-Dokumente akzeptiert.',
    uploadPdfTooLarge: 'Dieses PDF ist größer als 5 MB. Versuchen Sie einen kleineren Scan.',
    uploadImageTooLarge: 'Dieses Bild ist auch nach der Komprimierung zu groß.',
    uploadImageUnreadable: 'Dieses Bild konnte nicht gelesen werden.',
    uploadFailed: 'Der Upload ist fehlgeschlagen. Prüfen Sie Ihre Verbindung und versuchen Sie es erneut.',
    addOrRenew: 'Dokument hinzufügen oder erneuern',
    expires: 'Läuft ab am',
    superseded: 'Ersetzt',
    verification: {
      PENDING: 'Ausstehend',
      VERIFIED: 'Verifiziert',
      REJECTED: 'Abgelehnt',
    },
    unknownType: 'Dokument',
    documentTypes: {
      CERTIFICATE: 'Berufszertifikat',
      LICENSE: 'Lizenz',
      PASSPORT: 'Reisepass',
      GHANACARD: 'Ghana Card',
      DRIVERLICENSE: 'Führerschein',
      VOTERCARD: 'Wählerausweis',
      PASSPHOTO: 'Passfoto',
      NHIS: 'NHIS-Karte',
      OTHER: 'Sonstiges',
    },
  },
  tabs: {
    today: 'Heute',
    messages: 'Nachrichten',
    documents: 'Dokumente',
    me: 'Ich',
  },
  dashboard: {
    title: 'Übersicht',
    open: 'Übersicht',
    savedData: 'Gespeicherte Zahlen werden angezeigt',
    savedDataOffline: 'Offline — gespeicherte Zahlen werden angezeigt',
    patients: 'Patienten',
    allPatients: 'Alle Patienten',
    children: 'Kinder',
    female: 'Weiblich',
    male: 'Männlich',
    cases: 'Fälle',
    openCases: 'Offen',
    urgentCases: 'Dringend',
    closedCases: 'Abgeschlossen',
    casesUnavailable: 'Fallzahlen sind derzeit nicht verfügbar. Ein Strich bedeutet unbekannt, nicht null.',
    noCharts: 'Diagramme und Einnahmen finden Sie im Webportal.',
  },
  cases: {
    title: 'Fälle',
    open: 'Meine Fälle',
    savedData: 'Gespeicherte Fälle werden angezeigt',
    savedDataOffline: 'Offline — gespeicherte Fälle werden angezeigt',
    empty: 'Keine Fälle in Ihrer Liste.',
    loadFailed: 'Ihre Fälle konnten nicht geladen werden.',
    filterAll: 'Alle',
    filterOpen: 'Offen',
    filterUrgent: 'Dringend',
    filterClosed: 'Abgeschlossen',
    countOpen: 'Offen',
    countUrgent: 'Dringend',
    countClosed: 'Abgeschlossen',
    noBrief: 'Fall ohne Titel',
    loadingMore: 'Weitere werden geladen',
    detailFailed: 'Dieser Fall konnte nicht geöffnet werden.',
    symptoms: 'Symptome',
    diagnosis: 'Diagnose',
    opened: 'Eröffnet am',
    detail: 'Fall',
    brief: 'Zusammenfassung',
    status: 'Status',
    save: 'Änderungen speichern',
    saveQueued: 'Auf diesem Telefon gespeichert. Es wird gesendet, sobald Sie Empfang haben.',
    noPermission: 'Ihre Rolle darf keine Fälle bearbeiten.',
    archive: 'Diesen Fall archivieren',
    archiveReason: 'Warum archivieren Sie ihn?',
    archiveHint: 'Der Fall bleibt in der Patientenakte. Er verschwindet aus Ihrer Liste, sobald gesendet wurde.',
    archivePending: 'Wird archiviert',
    archiveDoctorOnly: 'Nur eine Ärztin oder ein Arzt kann einen Fall archivieren.',
    archivedOn: 'Archiviert am',
    archivedNote: 'Dieser Fall ist nicht mehr aktiv.',
    close: 'Schließen',
  },
  patients: {
    title: 'Patienten',
    open: 'Meine Patienten',
    search: 'Nach Name oder Kennung suchen',
    savedData: 'Gespeicherte Patienten werden angezeigt',
    savedDataOffline: 'Offline — gespeicherte Patienten werden angezeigt',
    empty: 'Keine Patienten gefunden.',
    emptyNone: 'Sie haben noch keine Patienten.',
    loadFailed: 'Ihre Patienten konnten nicht geladen werden.',
    filterAll: 'Alle',
    filterFemale: 'Weiblich',
    filterMale: 'Männlich',
    filterChildren: 'Kinder',
    child: 'Kind',
    lastSeen: 'Zuletzt gesehen',
    neverSeen: 'Keine Aktivität erfasst',
    recencyRestricted: 'Ihre Rolle darf die letzte Aktivität nicht sehen',
    rowsRestricted: 'Einige Patienten werden nicht angezeigt: Ihre Rolle darf Fallzuweisungen nicht lesen.',
    recordsRestricted:
      'Ihre Rolle darf keine Patientenakten öffnen, daher lassen sich diese Zeilen nicht öffnen. Ein erneuter Versuch ändert daran nichts.',
    loadingMore: 'Weitere werden geladen',
    record: 'Akte',
    recordFailed: 'Diese Akte konnte nicht geladen werden.',
    recordOffline: 'Es wird die auf diesem Telefon gespeicherte Kopie angezeigt.',
    contact: 'Kontakt',
    dateOfBirth: 'Geburtsdatum',
    phone: 'Telefon',
    email: 'E-Mail',
    emergencyContact: 'Notfallkontakt',
    cases: 'Fälle',
    noCases: 'Keine offenen Fälle.',
    activity: 'Aktivität',
    noActivity: 'Keine Aktivität erfasst.',
    activityRestricted:
      'Ihre Rolle darf den Aktivitätsverlauf dieser Patientin bzw. dieses Patienten nicht lesen. Es können Einträge vorhanden sein, die hier nicht angezeigt werden.',
    medications: 'Medikamente',
    noMedications: 'Keine Medikamente erfasst.',
    reports: 'Berichte',
    noReports: 'Keine Berichte vorhanden.',
    cannotFile: 'Ihre Rolle darf keine Notizen zu einer Akte erfassen.',
    fileActivity: 'Eintrag hinzufügen',
    fileReport: 'Bericht hinzufügen',
    activityTitle: 'Was geschehen ist',
    activityDetail: 'Einzelheiten',
    reportName: 'Berichtsname',
    reportType: 'Art',
    file: 'Hinzufügen',
    fileQueued: 'Auf diesem Telefon gespeichert. Es wird gesendet, sobald Sie Empfang haben.',
    fileNeedsText: 'Beschreiben Sie zuerst, was geschehen ist.',
    noPermission: 'Ihre Rolle darf keine Notizen erfassen.',
    close: 'Schließen',
  },
  /**
   * Shift names, one key per value of `DUTY_ROSTER_SHIFTS` in `core/api/duty-roster-api.service`.
   *
   * Both screens that show a shift rendered `{{ round.shift | titlecase }}` until 2026-09-04, which
   * put the raw `DAY`/`OFF` on the card in title case — acceptable-looking English and nothing
   * sensible in the other three languages, which is precisely why nobody noticed. `shift-names.spec`
   * derives the expected keys from that array, so a value added there fails here in all four
   * languages rather than reaching a screen.
   *
   * Bare names, unlike `web/`'s window-bearing ones: both screens already render `windowText(shift)`
   * beside the name, so a label carrying the hours would print them twice.
   */
  shiftType: {
    DAY: 'Tag',
    EVENING: 'Abend',
    NIGHT: 'Nacht',
    OFF: 'Frei',
    FLEXIBLE: 'Flexibel',
  },
  roster: {
    title: 'Dienstplan',
    open: 'Meinen Dienstplan öffnen',
    savedData: 'Gespeicherte Daten werden angezeigt',
    savedDataOffline: 'Offline — gespeicherte Daten werden angezeigt',
    worked: 'Im Dienst',
    onLeave: 'Abwesend',
    noneThisYear: 'Für dieses Jahr ist nichts eingeplant.',
    loadFailed: 'Ihr Dienstplan konnte nicht geladen werden.',
    dayTitle: 'Touren',
    dayLoadFailed: 'Dieser Tag konnte nicht geladen werden. Dafür ist eine Verbindung nötig.',
    dayEmpty: 'An diesem Tag ist nichts eingeplant.',
    visits: 'Besuche',
    noVisits: 'Keine Besuche angegeben',
    unnamedCustomer: 'Angaben nicht verfügbar',
    close: 'Schließen',
    previousYear: 'Vorheriges Jahr',
    nextYear: 'Nächstes Jahr',
  },
  absence: {
    title: 'Abwesenheit',
    none: 'Keine Abwesenheit eingetragen.',
    request: 'Abwesenheit beantragen',
    from: 'Erster Tag',
    to: 'Letzter Tag',
    type: 'Grund',
    typeHoliday: 'Urlaub',
    typeSick: 'Krankheit',
    typeOther: 'Sonstiges',
    statusRequested: 'Beantragt',
    statusApproved: 'Genehmigt',
    submit: 'Antrag senden',
    withdraw: 'Zurückziehen',
    withdrawConfirm: 'Diese Abwesenheit zurückziehen?',
    requestFailed: 'Ihr Antrag konnte nicht gesendet werden.',
    withdrawFailed: 'Sie konnte nicht zurückgezogen werden.',
    datesInvalid: 'Der letzte Tag kann nicht vor dem ersten liegen.',
    willSendLater: 'Sie sind offline. Dies wird gesendet, sobald Sie Empfang haben.',
  },
  me: {
    birthDate: 'Geburtsdatum',
    sex: 'Geschlecht',
    cardType: 'Ausweisart',
    cardNumber: 'Ausweisnummer',
    address: 'Adresse',
    street: 'Straße',
    city: 'Stadt',
    region: 'Region',
    country: 'Land',
    nextOfKin: 'Notfallkontakt',
    kinName: 'Name',
    kinRelationship: 'Beziehung',
    kinPhone: 'Telefon',
    password: 'Passwort',
    currentPassword: 'Aktuelles Passwort',
    newPassword: 'Neues Passwort',
    confirmPassword: 'Neues Passwort bestätigen',
    changePassword: 'Passwort ändern',
    changePasswordDetail: 'Sie bleiben nach der Änderung auf diesem Gerät angemeldet.',
    passwordMismatch: 'Die beiden neuen Passwörter stimmen nicht überein.',
    passwordLength: 'Ein Passwort muss zwischen 4 und 100 Zeichen lang sein.',
    passwordChanged: 'Passwort geändert.',
    passwordWrong: 'Dieses aktuelle Passwort war nicht richtig.',
    completion: {
      title: 'Profil vollständig',
      complete: 'Alles liegt vor. Ein Administrator prüft als Nächstes Ihre Nachweise.',
      incomplete: 'Ihr Profil wird aktiv, sobald diese vollständig sind und ein Administrator sie geprüft hat.',
      requirements: {
        consent: 'Einwilligung',
        profile: 'Persönliche Angaben',
        address: 'Adresse',
        nextOfKin: 'Notfallkontakt',
        certificate: 'Zertifikat',
        license: 'Lizenz',
        identity: 'Ausweisdokument',
        photo: 'Passfoto',
      },
    },
    title: 'Ich',
    profile: 'Profil',
    firstName: 'Vorname',
    lastName: 'Nachname',
    email: 'E-Mail',
    phone: 'Mobiltelefon',
    saved: 'Profil gespeichert',
    saveFailed: 'Ihr Profil konnte nicht gespeichert werden',
    loadFailed: 'Ihr Profil konnte nicht geladen werden',
    notifications: 'Benachrichtigungen',
    pushMessages: 'Neue Nachrichten',
    pushCompliance: 'Nachweis-Erinnerungen',
    pushSenderName: 'Absender anzeigen',
    pushSenderNameDetail:
      'Benachrichtigungsvorschauen sind für jeden sichtbar, der Ihr Telefon in der Hand hält — Namen bleiben daher verborgen, solange Sie dies nicht aktivieren.',
    prefsSaveFailed: 'Ihre Benachrichtigungseinstellungen konnten nicht gespeichert werden',
    language: 'Sprache',
    shareRoster: 'Meinen Dienstplan teilen',
    shareUnavailable: 'Teilen ist auf diesem Gerät nicht verfügbar',
    noRoster: 'Keine bevorstehenden Einsätze zum Teilen',
    rosterTitle: 'Mein Dienstplan',
    unsent: 'Nicht gesendet',
    unsentDetail: '{{count}} Einträge wurden noch nicht gesendet.',
    unsentRetry: 'Erneut versuchen',
    unsentDiscard: 'Verwerfen',
    unsentConflict: 'Jemand anderes hat dies geändert. Öffnen Sie es und wenden Sie Ihre Änderung erneut an.',
    unsentRejected: 'Der Server hat dies abgelehnt.',
    unsentExpired: 'Dies wartet seit über einem Tag und wird nicht gesendet.',
    signOutBlocked: 'Sie haben nicht gesendete Einträge',
    signOutBlockedDetail: 'Beim Abmelden werden sie von diesem Telefon gelöscht. Senden oder verwerfen Sie sie zuerst.',
    signOutSendNow: 'Jetzt senden',
    signOutDiscardAndLeave: 'Verwerfen und abmelden',
    signOut: 'Abmelden',
    signOutConfirm: 'Von diesem Gerät abmelden?',
    signOutDetail: 'Zwischengespeicherte Patientendaten und Nachrichten werden von diesem Telefon gelöscht.',
  },
};

/** The languages this app ships, matching `web/`'s four exactly. */
export const SUPPORTED_LANGUAGES = ['en', 'es', 'fr', 'de'] as const;

export type SupportedLanguage = (typeof SUPPORTED_LANGUAGES)[number];

export const CATALOGUES: Record<SupportedLanguage, typeof EN> = { en: EN, es: ES, fr: FR, de: DE };

/** Shown in the language picker, each in its own language rather than translated. */
export const LANGUAGE_NAMES: Record<SupportedLanguage, string> = {
  en: 'English',
  es: 'Español',
  fr: 'Français',
  de: 'Deutsch',
};
